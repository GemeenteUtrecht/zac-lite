...
2. Get the code:
   ...
   .. code-block:: bash
       ...
       $ git clone git@github.com:GemeenteUtrecht/zac-lite.git
       $ cd backend
...
``src/zac_lite/conf/local.py``. You can base this file on the
...
* ``DB_NAME``: name of the database for the project. Defaults to ``zac_lite``.
* ``DB_USER``: username to connect to the database with. Defaults to ``zac_lite``.
* ``DB_PASSWORD``: password to use to connect to the database. Defaults to ``zac_lite``.
...
2. Start the database and web services:
   ...
   .. code-block:: bash
       ...
       $ docker-compose up -d
       Starting zac_lite_db_1 ... done
       Starting zac_lite_web_1 ... done
...
3. Create an admin user and load initial data. If different container names
   ...
   .. code-block:: bash
       ...
       $ docker exec -it zac_lite_web_1 /app/src/manage.py createsuperuser
       Username: admin
       ...
       Superuser created successfully.

       $ docker exec -it zac_lite_web_1 /app/src/manage.py loaddata admin_index groups
...
.. _Github: https://github.com/GemeenteUtrecht/zac-lite/
...
environment variables. See ``src/zac_lite/conf/docker.py`` for
all settings.

.. code-block:: bash

    $ docker build -t zac_lite
    $ docker run \
        -p 8000:8000 \
        -e DATABASE_USERNAME=... \
        -e DATABASE_PASSWORD=... \
    ...
    $ docker exec -it zac_lite /app/src/manage.py createsuperuser
